...
'''
'''
def simluate_rayleigh_fading_channel( nrof_samples, avg_snr_dB, awgn_data, packet_sizes, norm_doppler = 0.01, 
...
def run_simulation(pars, nrof_ttis):
    ...
    dts_bandit = DiscountThompsonSamplingBandit(nrof_rates, packet_sizes, pars['dts_discount_factor'])
...
class BaseModelFreeBandit():
    ...
    # Update the bandit
    def update(self, rate_index, cqi, ack):
        """CQI is unused. It is only for compatibility with the other bandits."""
        if ack:
...
class DiscountThompsonSamplingBandit(BaseModelFreeBandit):
    """The reeiver only receives ACK signal, not CQI. Each MCS index is formulated as an arm."""
    def __init__(self, 
                 nrof_rates, 
                 packet_sizes,
                 discount = 1):
